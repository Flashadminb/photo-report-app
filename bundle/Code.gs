...
var CFG = {
  ...
  // วัสดุสิ้นเปลือง — แยกไฟล์อีกใบ ไม่ปนกับชีทงานประจำวัน
  SUP_ID:    '1IdEApg6zoZI1pVZZf5MJWefuPvxZJYtXHIn3kpayx38', // วัสดุสิ้นเปลือง
  ...
  // ── ชื่อชีทในไฟล์วัสดุสิ้นเปลือง ──────────────────────────────────────
  SUP: {
    STOCK: 'คงคลัง',
    LOG:   'ประวัติการเบิก'
  },
  SUP_HEAD: { STOCK: 1, LOG: 1 },   // หัวตารางอยู่แถว 1 ทั้งสองชีท
  ...
  COL: {
    ...
    // วัสดุ!คงคลัง — ทะเบียนของ ไม่มีช่อง "คงเหลือ"
    // คงเหลือคำนวณจากสมุดเสมอ ตัวเลขที่เก็บไว้เฉย ๆ มีวันเพี้ยนโดยไม่มีใครรู้
    SSTOCK: {
      CODE: 1, NAME: 2, UNIT: 3, CAT: 4, START: 5, REORDER: 6, STATUS: 7, NOTE: 8
    },
    // วัสดุ!ประวัติการเบิก — 1 รายการ = 1 แถว เบิกทีเดียว 3 อย่างก็ 3 แถว
    SLOG: {
      DATE: 1, TIME: 2, KIND: 3, CODE: 4, NAME: 5, QTY: 6, UNIT: 7, LEFT: 8,
      EMP_ID: 9, EMP_NAME: 10, DEPT: 11, NOTE: 12, PROOF: 13, REF: 14
    }
  },
  ...
  V: {
    ...
    // ชนิดแถวในสมุดวัสดุ — จำนวนเป็นบวกเสมอ ช่องนี้บอกทิศทางแทน
    // ไม่ใช้เลขติดลบ เพราะคนอ่านชีทเองจะงงและบวกผิด
    SUP_OUT: 'เบิก',
    SUP_IN:  'รับเข้า',
    SUP_UP:  'ปรับเพิ่ม',
    SUP_DN:  'ปรับลด'
  ...
  // รายการที่สั่งให้ "ถือว่าคืนครบแล้ว" ไม่ว่าชีทจะบอกว่าคืนไม่ครบก็ตาม
  //
  // กติกาใหม่คือคืนไม่ครบให้ค้างเฉพาะตัวที่ยังไม่คืน ซึ่งถูกต้องกับของจริง
  // แต่ของค้างเก่าบางใบเป็นข้อมูลที่ตกลงกันแล้วว่าไม่ต้องตามอีก จึงปิดปากใบไว้ตรงนี้
  // ใส่รหัสรายการเพิ่มได้เรื่อย ๆ เป็นการซ่อนจากหน้าค้างคืนเท่านั้น ไม่ได้ลบข้อมูล
  PARTIAL_SKIP: [
    'PP-20260829-2749',      // สิริมา ฉาบพิมาย  4 คัน
    'IDATA-20260827-2610',   // วรรณวิสา สิงห์คำ 6 เครื่อง
    'IDATA-20260824-2361',   // เนรัชญา แม้นวิลัย 2 เครื่อง
    'IDATA-20260827-2633',
    'IDATA-20260825-2395',
    'IDATA-20260816-1870',
    'PP-20260814-1735'
...
function writeCardIssue_(items, why, giver, trained, proof) {
  ...
  var rows = items.map(function (x) {
    ...
    // ช่องหลักฐาน = ลิงก์โฟลเดอร์เดียว เก็บทั้งรูปหน้าคนที่รับบัตรและรูปการอบรมรวมกัน
    // ทุกแถวในรอบเดียวกันชี้โฟลเดอร์เดียวกัน เปิดจากแถวไหนก็เห็นครบทั้งชุด
...
function sessionPayload_(m, u) {
  ...
  // ใครเห็นรายการค้างคืนของใครบ้าง
  //   แอดมิน          เห็นทั้งโรง
  //   หัวหน้างาน      เห็นเฉพาะคนในแผนกตัวเอง (ตั้งแผนกเป็น "ทุกแผนก" ถึงจะเห็นทั้งโรง)
  //   พนักงานหน้างาน  เห็นแต่ของตัวเอง
  //
  // ของเดิมหัวหน้างานเห็นทั้งโรง ซึ่งกว้างเกินงานจริง หัวหน้าอยากรู้ว่า
  // "ลูกน้องกะฉันใครยังไม่คืน" ไม่ใช่ "ทั้งโรงใครยังไม่คืน" ที่ยาวจนไม่มีใครอ่าน
  var isLead = (u.role === CFG.V.ROLE_LEAD);
  var mine;
  if (isAdmin || (isLead && u.dept === 'ทุกแผนก')) {
    mine = open;
  } else if (isLead) {
    mine = open.filter(function (j) { return s_(j.dept) === u.dept; });
  } else {
    mine = open.filter(function (j) { return j.empId === u.id; });
  }
...
function prepareSubmit_(p) {
  ...
  // แอดมินบันทึกแทนคนอื่น ได้ทั้งตอนเบิกและตอนคืน
  //
  // รายการยังเป็นของเจ้าตัวเสมอ ไม่ใช่ของแอดมิน — คอลัมน์รหัสพนักงาน/ชื่อ/แผนก
  // คงเป็นคนที่ถือของจริง ส่วนแอดมินไปอยู่ช่อง "ดำเนินการโดย" แยกต่างหาก
  // ประวัติจึงตอบได้ทั้งสองคำถาม: ของอยู่กับใคร และใครเป็นคนกดบันทึกให้
  //
  // เคยห้ามบันทึกแทนตอนคืนไว้ แต่ของจริงแอดมินต้องปิดรายการให้ลูกน้องได้
  // เช่นคนลาออก ลืมกดคืน หรือคืนของแล้วแต่ไม่ได้บันทึก
  var owner = u, actedBy = '';
